...
func _on_Tween_tween_all_completed():
	mod = 0 if mod == modulate_target else modulate_target
	sca = Vector2(.5, .5) if sca == scale_target else scale_target
	
	$Tween.interpolate_property($Highlight, "scale", $Highlight.scale, sca, .5, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.interpolate_property($Highlight, "scale", $Highlight.scale, sca, .5, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.start()
